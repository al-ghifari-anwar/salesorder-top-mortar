...
class Jadwalvisit extends CI_Controller
{
    ...
    public function printAdmin()
    {
        ...
        $mpdf = new \Mpdf\Mpdf(['format' => 'A4']);
        $mpdf->SetMargins(0, 0, 5);
        $html = $this->load->view('Jadwalvisit/PrintAdmin', $data, true);
        $mpdf->AddPage('L');
        $mpdf->WriteHTML($html);
        $mpdf->Output();
        // $mpdf->Output($filePath, \Mpdf\Output\Destination::FILE);
    ...
    public function sendNotif()
    {
        ...
        foreach ($citys as $city) {
            ...
            // Buat direktori penyimpanan sementara
            $folderPath = FCPATH . 'assets/tmp/jadwal_visit/';
            // Nama file berdasarkan invoice ID + timestamp
            $fileName = "jadwal_visit_" . $city['nama_city'] . '_' . date('d F Y') . '_' . time() . '.pdf';
            $filePath = $folderPath . $fileName;
            $fileUrl = "https://order.topmortarindonesia.com/assets/tmp/jadwal_visit/" . $fileName;
