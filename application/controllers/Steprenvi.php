...
class Steprenvi extends CI_Controller
{
    ...
    public function save()
    {
        ...
        foreach ($citys as $city) {
            ...
            foreach ($jatem1s as $jatem1) {
                ...
                $id_sj = $jatem1['id_surat_jalan'];

                $sj = $this->db->where('id_surat_jalan', $id_sj)->get('tb_surat_jalan')->row_array();
                ...
                $jatem = date('Y-m-d', strtotime("+30" . " days", strtotime($jatem1['date_invoice'])));

                if ($sj['is_cod'] == 1) {
                    $jatem =  date('Y-m-d', strtotime("+1" . " days", strtotime($jatem1['date_invoice'])));
                } else {
                    if ($jatem1['termin_payment'] != 30) {
                        $jatem = date('Y-m-d', strtotime("+" . $jatem1['termin_payment'] . " days", strtotime($jatem1['date_invoice'])));
                    }
                }

                $date1 = new DateTime(date("Y-m-d"));
                $date2 = new DateTime($jatem);
            ...
            foreach ($jatem2s as $jatem2) {
                ...
                $jatem = date('Y-m-d', strtotime("+30" . " days", strtotime($jatem2['date_invoice'])));

                if ($sj['is_cod'] == 1) {
                    $jatem =  date('Y-m-d', strtotime("+1" . " days", strtotime($jatem2['date_invoice'])));
                } else {
                    if ($jatem2['termin_payment'] != 30) {
            ...
            foreach ($jatem3s as $jatem3) {
                ...
                $jatem = date('Y-m-d', strtotime("+30" . " days", strtotime($jatem3['date_invoice'])));

                if ($sj['is_cod'] == 1) {
                    $jatem =  date('Y-m-d', strtotime("+1" . " days", strtotime($jatem3['date_invoice'])));
                } else {
                    if ($jatem3['termin_payment'] != 30) {
            ...
            foreach ($mingguans as $mingguan) {
                ...
                $id_sj = $mingguan['id_surat_jalan'];

                $sj = $this->db->where('id_surat_jalan', $id_sj)->get('tb_surat_jalan')->row_array();

                $count = $this->db->query("SELECT COUNT(*) AS jmlRenvis FROM tb_renvis_jatem WHERE id_invoice = '$id_inv' AND type_renvis = 'mingguan'")->row_array();

                $jatem = date('Y-m-d', strtotime("+30" . " days", strtotime($mingguan['date_invoice'])));

                if ($sj['is_cod'] == 1) {
                    $jatem =  date('Y-m-d', strtotime("+1" . " days", strtotime($mingguan['date_invoice'])));
                } else {
                    if ($mingguan['termin_payment'] != 30) {
            ...
            foreach ($passives as $passive) {
                ...
                $passive['id_renvis_jatem'] = $passive['id_rencana_visit'];
            ...
            foreach ($janjiBayars as $janjiBayar) {
                ...
                if ($invoice['status_invoice'] != 'paid') {
                    // Jatem Days
                    $jatemInv = date('Y-m-d', strtotime("+30" . " days", strtotime($invoice['date_invoice'])));

                    if ($invoice['is_cod'] == 1) {
                        $jatemInv =  date('Y-m-d', strtotime("+1" . " days", strtotime($invoice['date_invoice'])));
                    } else {
                        if ($invoice['termin_payment'] != 30) {
            ...
            foreach ($renvis as $renvi) {
                ...
                $renvisFilter = [
                    'id_renvi' => $renvi['id_renvis_jatem'],
                    'id_contact' => $renvi['id_contact'],
                    'number' => 2,
                    'source' => $renvi['type_renvis'],
                    'nama' => $renvi['nama'],
            ...
            foreach ($renvisPassives as $renvisPassive) {
                ...
                $renvisFilter = [
                    'id_renvi' => $renvisPassive['id_renvis_jatem'],
                    'id_contact' => $renvisPassive['id_contact'],
                    'number' => 5,
                    'source' => $renvisPassive['type_renvis'],
            ...
            foreach ($jadwalVisits as $jadwalVisit) {
                $stepRenviData = [
                    ...
                    'days_step_renvi' => $jadwalVisit['days'],
                    'days_jatem_step_renvi' => $jadwalVisit['daysJatem'],
                    'total_invoice' => $jadwalVisit['total_invoice'],
                    'is_active' => $jadwalVisit['is_active'],
                ];

                $save = $this->db->insert('tb_step_renvi', $stepRenviData);
