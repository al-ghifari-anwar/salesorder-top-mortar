...
class MPayment extends CI_Model
{
    ...
    public function getByIdInvoiceOnly($id_invoice)
    {
    ...
    public function getUnmatch()
    {
        ...
        if ($this->session->userdata('id_distributor') == 1) {
            ...
            $this->db->where('source', '8881051362');
        } else if ($this->session->userdata('id_distributor') == 9) {
            $this->db->where('source', 'mojokerto');
        } else if ($this->session->userdata('id_distributor') == 10) {
            $this->db->where('source', '8880762231');
        }
    ...
    public function setPaymentInv($id)
    {
        ...
        $id_invoice = isset($post['id_invoice']) ? $post['id_invoice'] : 0;
        ...
        if ($query) {
            ...
            if ($getInv['total_invoice'] == $getTotalPayment['amount_total']) {
                ...
                if ($setInvStatus) {
                    $removeRenvis = $this->db->update('tb_rencana_visit', ['is_visited' => 1, 'visit_date' => $date], ['id_invoice' => $getInv['id_contact'], 'type_rencana' => 'jatem']);

                    $removeRenvisWeekly = $this->db->update('tb_rencana_visit', ['is_visited' => 1, 'visit_date' => $date], ['id_invoice' => $getInv['id_contact'], 'type_rencana' => 'tagih_mingguan']);
