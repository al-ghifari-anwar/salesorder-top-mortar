...
class MDetailSuratJalan extends CI_Model
{
    ...
    public function getSoldItemsGlobalForTotal($id_city = null)
    {
        $this->db->select("nama_produk, SUM(qty_produk) AS qty_produk, tb_produk.id_produk, name_satuan");
        $this->db->join('tb_produk', 'tb_produk.id_produk = tb_detail_surat_jalan.id_produk');
        $this->db->join('tb_surat_jalan', 'tb_surat_jalan.id_surat_jalan = tb_detail_surat_jalan.id_surat_jalan');
    ...
    public function getSoldItemsByDateGlobalForTotal($id_city = null, $dateFrom = null, $dateTo = null)
    {
        $this->db->select("nama_produk, SUM(qty_produk) AS qty_produk, tb_produk.id_produk, name_satuan");
        $this->db->join('tb_produk', 'tb_produk.id_produk = tb_detail_surat_jalan.id_produk');
        $this->db->join('tb_surat_jalan', 'tb_surat_jalan.id_surat_jalan = tb_detail_surat_jalan.id_surat_jalan');
    ...
    public function getClosingItemByIdContact($id_contact, $lastMonth)
    {
        $this->db->select("nama_produk, SUM(qty_produk) AS qty_produk, tb_produk.id_produk");
        $this->db->join('tb_produk', 'tb_produk.id_produk = tb_detail_surat_jalan.id_produk');
        $this->db->join('tb_surat_jalan', 'tb_surat_jalan.id_surat_jalan = tb_detail_surat_jalan.id_surat_jalan');
        $this->db->join('tb_city', 'tb_city.id_city = tb_produk.id_city');
        $query = $this->db->get_where('tb_detail_surat_jalan', ['tb_surat_jalan.is_closing' => 1, 'tb_surat_jalan.id_contact' => $id_contact, 'MONTH(tb_surat_jalan.date_closing) =' => $lastMonth])->row_array();
    ...
    public function setBonusItem($id_surat_jalan, $id_promo, $id_city)
    {
        ...
        if ($id_promo != 0) {
            ...
            $items = $this->db->get_where('tb_detail_surat_jalan', ['id_surat_jalan' => $id_surat_jalan, 'is_bonus' => 0])->result_array();
                ...
                if ($masterProduk['is_default_promo'] == 1) {
                    ...
                    if (floor($multiplier) > 0) {
                        if ($this->session->userdata('id_distributor') != 6) {
                            ...
                            $this->id_surat_jalan = $id_surat_jalan;
                            $this->id_produk = $item['id_produk'];
                            $this->qty_produk = floor($multiplier) * $masterProduk['bonus_promo'];
                            $this->price = $item['harga_produk'];
                            $this->amount = 0;
                            $this->is_bonus = 1;
