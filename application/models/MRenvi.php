...
class MRenvi extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT tb_rencana_visit.*, tb_contact.nama, tb_contact.nomorhp, tb_contact.id_city, tb_contact.store_status, tb_contact.store_owner, tb_contact.maps_url, tb_contact.created_at AS created_at_store, tb_contact.reputation, date_invoice, termin_payment, tb_invoice.id_invoice, tb_contact.pass_contact, tb_contact.hari_bayar FROM tb_rencana_visit JOIN tb_contact ON tb_contact.id_contact = tb_rencana_visit.id_contact JOIN tb_invoice ON tb_invoice.id_invoice = tb_rencana_visit.id_invoice WHERE type_rencana = 'tagih_mingguan' AND tb_contact.id_city = '$id_city' AND is_visited = 0 AND AND tb_contact.cluster = '$cluster' tb_invoice.status_invoice = 'waiting' GROUP BY tb_rencana_visit.id_contact")->result_array();
