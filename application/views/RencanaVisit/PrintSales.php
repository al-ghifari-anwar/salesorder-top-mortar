        ...
        <?php if ($user != null) : ?>
            <?php foreach ($user as $user) : ?>
                ...
                foreach ($dateGroup as $dateGroup) : ?>
                    ...
                    $visitByDate = $this->db->get_where('tb_visit', ['id_user' => $dateGroup['id_user'], 'DATE(date_visit)' => date("Y-m-d", strtotime($dateGroup['date_visit'])), 'is_deleted' => 0, 'tb_contact.id_city' => $id_city, 'is_approved' => 1])->result_array();

                    $this->db->join('tb_contact', 'tb_contact.id_contact = tb_visit.id_contact');
                    $this->db->select("COUNT(*) AS total_visit");
                    $this->db->group_by('tb_visit.id_contact');
                    $getTotal = $this->db->get_where('tb_visit', ['id_user' => $id_user, 'DATE(date_visit)' => date("Y-m-d", strtotime($dateGroup['date_visit'])), 'is_deleted' => 0, 'tb_contact.id_city' => $id_city, 'is_approved' => 1])->num_rows();
                    $total += $getTotal;
                    ?>
